...
@interface SKFResourceLoader ()<SKFAudioDownLoaderDelegate>

@property (nonatomic, strong) SKFAudioDownLoader *downLoader;
...
@implementation SKFResourceLoader

- (SKFAudioDownLoader *)downLoader {
    if(!_downLoader) {
        _downLoader = [[SKFAudioDownLoader alloc] init];
...
- (void)handleAllRequest {
    ...
    for (AVAssetResourceLoadingRequest *loadingRequest in self.loadingRequests) {
        ...
        NSData *data = [NSData dataWithContentsOfFile:[SKFAudioFileTool tmpPathWithURL:url] options:NSDataReadingMappedIfSafe error:nil];
        if (data.length == 0) {
            data = [NSData dataWithContentsOfFile:[SKFAudioFileTool cachePathWithURL:url] options:NSDataReadingMappedIfSafe error:nil];
        }
...
- (BOOL)resourceLoader:(AVAssetResourceLoader *)resourceLoader shouldWaitForLoadingOfRequestedResource:(AVAssetResourceLoadingRequest *)loadingRequest
{
    ...
    if ([SKFAudioFileTool isCacheFileExists:url])
    {
        // 三个步骤, 直接响应数据
...
- (void)handleRequestWithLoadingRequest: (AVAssetResourceLoadingRequest *)loadingRequest {
    NSURL *url = [loadingRequest.request.URL httpURL];
    // 1. 填充信息头
    loadingRequest.contentInformationRequest.contentType = [SKFAudioFileTool contentTypeWithURL:url];
    loadingRequest.contentInformationRequest.contentLength = [SKFAudioFileTool cacheFileSizeWithURL:url];
    loadingRequest.contentInformationRequest.byteRangeAccessSupported = YES;
    
    
    // 2. 响应数据
    NSData *data = [NSData dataWithContentsOfFile:[SKFAudioFileTool cachePathWithURL:url] options:NSDataReadingMappedIfSafe error:nil];
