...
@interface SKFRemotePlayer ()
{
    BOOL _isUserPause;
}
@property (nonatomic, strong) AVPlayer *player;

@property (nonatomic, strong) SKFResourceLoader *resourceLoader;

@end


@implementation SKFRemotePlayer

static SKFRemotePlayer *_shareInstance;

+ (instancetype)shareInstance {
    if (!_shareInstance) {
        _shareInstance = [[SKFRemotePlayer alloc] init];
    }
...
//- (SKFResourceLoader *)resourceLoader {
//    if (!_resourceLoader) {
//        _resourceLoader = [[SKFResourceLoader alloc] init];
//    }
...
- (void)playWithURL: (NSURL *)url isCache:(BOOL)isCache {
    ...
    if ([self.url isEqual:url]) {
        ...
        if (self.state == SKFRemotePlayerStatePlaying) {
            return;
        }
        if (self.state == SKFRemotePlayerStatePause) {
            [self resume];
            return;
        }
        if (self.state == SKFRemotePlayerStateLoading) {
            return;
        }
    ...
    self.resourceLoader = [[SKFResourceLoader alloc] init];
...
- (void)pause{
    ...
    if (self.player) {
        _isUserPause = YES;
        self.state = SKFRemotePlayerStatePause;
    }
}
...
- (void)resume{
    ...
    if (self.player && self.player.currentItem.playbackLikelyToKeepUp) {
        _isUserPause = NO;
        self.state = SKFRemotePlayerStatePlaying;
    }
...
- (void)stop{
    ...
    [self clearObserver:self.player.currentItem];
    self.player = nil;
    self.state = SKFRemotePlayerStateStopped;
}
...
- (void)setState:(SKFRemotePlayerState)state {
    //    if (_state == state) {
    //        return;
    //    }
    _state = state;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
    ...
    if ([keyPath isEqualToString:@"status"]) {
        ...
        switch (status) {
            ...
            {
                NSLog(@"数据准备失败, 无法播放");
                self.state = SKFRemotePlayerStateFailed;
                break;
            }
            ...
            {
                NSLog(@"未知");
                self.state = SKFRemotePlayerStateUnknown;
                break;
            }
        }
    ...
    if ([keyPath isEqualToString:@"playbackLikelyToKeepUp"]) {
        ...
        if (playbackLikelyToKeepUp) {
            ...
            NSLog(@"数据不够播放");
            self.state = SKFRemotePlayerStateLoading;
        }
    }
